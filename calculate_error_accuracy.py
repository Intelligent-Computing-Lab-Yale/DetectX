...
parser.add_argument('--pgd_params', default='0.125,0.007,7')
parser.add_argument('--fgsm_eps', type=float, default='0.007')
...
parser.add_argument('--cw_params', default='0.125,0.007,7')
...
if args.type == 'cifar10':
    ...
    # model_infer.load_state_dict(torch.load('./adv_train_pgd8/adv_train_pgd8.pth'))
    ...
    model_infer = models.vgg16_bn() #tinyimagenet_net.ResNet().cuda()
    model_infer.classifier = nn.Sequential(nn.Linear(in_features=512, out_features=512, bias=True),
                                   nn.ReLU(inplace=True), nn.Dropout(p=0.5),
                                   nn.Linear(in_features=512, out_features=256, bias=True),
                                   nn.ReLU(inplace=True), nn.Dropout(p=0.5),
    ...
    bin_arr = bin_arr[:300]
    list_probs = lut[1]
    # model_infer.load_state_dict(torch.load('./cifar100/adv_train_pgd8_new/cifar100_adv_train_pgd8_new.pth'))
...
if args.a_type == 'fgsm':
    ...
    adv_dataset_path = args.path+'/adv_test_set_'+args.type+'_'+args.a_type+'_e=' + str(eps)
    soi_path = args.path + '/soi_' + args.type + '_' + args.a_type + '_e=' + str(eps)
...
if args.a_type == 'pgd':
    ...
    soi_path = args.path + '/soi_' + args.type + '_' + args.a_type+'_e=' + str(eps)+'_a=' + str(alpha)+'_n=' + str(steps)
...
for i, (data, labels_tru) in enumerate(test_loader):
    ...
    if args.clean != 1:
        inp_adv = pgd.pgd_attack(model, data.cuda(), labels_tru.cuda(), eps, alpha, steps )#attack(data, labels_tru)
    else:
        inp_adv = data.cuda()
        ...
        if args.baseline == 0:
            for idx, si in enumerate(soi):
                ...
                if args.clean == 1 and rand_no <= prob_clean:
                    pred = output[idx,:].data.max(1)[1]
                    accuracy += pred.cpu().neq(indx_target[idx]).sum()
